...
const adminController = {
    getPending: async(req, res) => {
        ...
        const user = await User.findOne({ username: verifyToken.data.username }).lean();
        let pendingAccount = await Account.find({ status: "pending" }).lean().select("username");
        let pendingUser = []
        ...
        if (req.query.search == 'true') {
            ...
            User.find({ $and: [{ username: { $in: pendingUser } }, generateSearchUserFilter(req)] })
                .lean()
                .exec((err, result) => {
                    if (err !== null) {
    ...
    getPendingTransaction: async(req, res) => {
        ...
        if (req.query.search == 'true') {
                    ...
                    if (err !== null) {
                        ...
                        if (result.length >= 1) {
                            ...
                            result.forEach((transaction) => {
                                ...
                                transaction.transactionDate = transaction['transactionDate'].toLocaleString("en-GB").replace(",", " -")
                                transaction.transactionAmount = toMoney(transaction.transactionAmount)
                            })
                            noData.status = false
                        }
            ...
            Transaction.find({ "status": "Pending" }).sort({ transactionDate: 1 }).lean().exec((err, result) => {
    ...
    acceptPendingTransaction: async(req, res) => {
        ...
        try {
            Transaction.findById({ _id: new ObjectId(`${req.params.transactionID.toString()}`) }, (err, result) => {
                    ...
                    if (err !== null) {
                        ...
                        Account.findOne({ username: result.userID }, (err, userData) => {
                            ...
                            if (userData.balance >= result.transactionAmount) {
                                Account.updateOne({ username: userData.username }, { $inc: { balance: result.transactionAmount * (-1) } }, (err, response) => {
    ...
    searchPendingTransaction: (req, res) => {
        ...
        if (req.query.search == 'true') {
            Transaction.find(generateSearchTransactionFilter(req)).sort({ transactionDate: 1 })
                .lean()
                .exec((err, result) => {
    ...
    searchPendingUser: async(req, res) => {
        ...
        if (req.query.search == 'true') {
            ...
            let account = await Account.find({ status: "pending" }).lean().select("username");
            let pendingAccount = []

            account.forEach((user) => {
                pendingAccount.push(user.username)
            })
            User.find({ $and: [{ username: { $in: pendingAccount } }, generateSearchUserFilter(req)] })
                .lean()
                .exec((err, result) => {
                    if (err !== null) {
                        return res.json({ msg: "Xảy ra lỗi trong quá trình tìm kiếm" })
...
function generateSearchTransactionFilter(req) {
    ...
    if (req.query.from && req.query.to) {
        filter.transactionDate = { $gte: new Date(req.query.from), $lt: new Date(req.query.to) }
